...
while ($q_row = mysqli_fetch_array($q_result)) {
    ...
    $sql4 = "SELECT COUNT(*)AS count_num,q_id FROM answer GROUP BY q_id ORDER BY count_num DESC";
    $count_result = mysqli_query($con, $sql4);
    while ($c_row = mysqli_fetch_array($count_result)) {
        $q_id = $c_row["q_id"];
        if ($q_id_each == $q_id) {
            $count_each = $c_row["count_num"];
            break;
        } else {
            $count_each = 0;
        }
    ...
    $arr_each = array("q_id" => $q_id_each, "q_title" => $q_title_each, "q_content" => $q_content_each, "crateTime" => $q_createtime_each, "tags_name" => $tag_arr, "user_name" => $user_name, "count" => $count_each);
    $arr[] = $arr_each;
}
